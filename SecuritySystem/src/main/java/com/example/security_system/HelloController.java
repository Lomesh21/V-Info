...
public class HelloController{
    ...
    void Login(ActionEvent event) {
        if(user.getText().equals(Username)&&pass.getText().equals(Password)){
            ...
            Image icon=new Image("C:\\Users\\Lomesh Wagh\\Desktop\\SecuritySystem\\src\\main\\java\\icon.png");
    ...
    void checking() throws ClassNotFoundException, SQLException {
        Class.forName("oracle.jdbc.driver.OracleDriver");
        con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:ORCL" , "Security","123124136");
        query= "select name from Basic_details where ID_no= '"+idn.getText()+"' and ID_type='"+idt.getText()+"'";
        pst=con.prepareStatement(query);
        rs= pst.executeQuery();
        if(!rs.next())
        {
...
//        con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:ORCL" , "Security","123124136");
            ...
            JOptionPane.showMessageDialog(null,"Name doesnot match");
        } else {
            JOptionPane.showMessageDialog(null,"Username or Password is invalid!!");
            clear();
        }
    ...
    void update(ActionEvent event) throws ClassNotFoundException, SQLException {
        if (user.getText().equals(Username) && pass.getText().equals(Password)) {
            ...
            query = "update display set display.Name='" + name.getText() + "', display.Age=" + age.getText() + " where display.ID_no= '" + idn.getText() + "' and display.ID_type='" + idt.getText() + "'";
            pst = con.prepareStatement(query);
            rs = pst.executeQuery();
            if(!rs.next()) {
                JOptionPane.showMessageDialog(null,"something went wrong");
